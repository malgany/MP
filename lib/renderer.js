...
async function ensureWindow(providedWindow) {
    if (providedWindow) {
        return providedWindow;
    }
    // Tenta carregar via require (CJS); se falhar por ESM ou ausência, tenta import() dinâmico
    try {
        // eslint-disable-next-line global-require
        ...
        try {
            ...
            const reqMsg = _requireErr && _requireErr.message ? _requireErr.message : String(_requireErr || "");
            const impMsg = _importErr && _importErr.message ? _importErr.message : String(_importErr || "");
            throw new Error(
                `renderWidget* requires a DOM-like environment. Install 'jsdom' or provide a custom window via options.window. [require error: ${reqMsg}] [import error: ${impMsg}]`
            );
        }
    }
}
...
async function ensurePreviewRuntime(options = {}) {
    if (cachedRuntime) {
        return cachedRuntime;
    }
    const windowObject = await ensureWindow(options.window);
...
async function renderWidgetToDom(viewSource, state = {}, options = {}) {
    const runtime = await ensurePreviewRuntime(options);
...
async function getPreviewApi(options = {}) {
    const runtime = await ensurePreviewRuntime(options);
